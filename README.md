...
Each iBeacon comes with a unique pair of major and minor number that can be retrieved via the [Estimote Cloud](https://cloud.estimote.com). We will use the pair major and minor number as our iBeacon's identifier. Assign also a random string as the secret token. This secret token will be used to verify ownership of the iBeacon.  [Update the _"identifier"_ and random _"secret token"_.](../../blob/master/Hook/app.js#L3-L4)
